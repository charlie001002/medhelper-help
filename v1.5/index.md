...
Go to **Settings**, open **Data: backup, restore & wipe data**, and tap **Back up my data**. MedHelper saves a copy of your medicines, doses, and history to the Files app (On My iPhone → MedHelper). From there you can keep it, or share it to email, AirDrop, or iCloud Drive.

### How do I restore a backup?
In **Settings → Data**, tap **Restore from backup** and choose your backup file. MedHelper shows how many prescriptions and dose records it contains, and its date, before you confirm.

Restore is intended for a fresh start — a new phone, or after reinstalling. If you already have recorded doses in the app, Restore is blocked so it cannot overwrite your history. If you genuinely want to replace everything, use **Wipe data** first, then restore.

### Why does a medicine say "Stock from backup, now out of date"?
A backup records your stock as it was when you made it. If doses were due between then and now, the restored stock may be too high. MedHelper flags those medicines so you can check the real amount. The message disappears once you update that medicine's stock.

### What does "Wipe data" do?
**Settings → Data → Wipe data** erases all your medicines, doses, and history. You choose:

- **Back up first, then delete** (recommended) — saves a backup and keeps your name and mode, so you can restore later.
- **Wipe everything, no backup** — also clears your name and mode, returning MedHelper to a fresh start.

You must type DELETE to confirm either way.
